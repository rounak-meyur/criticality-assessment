  ...
  if (length(nodes) > 1) {
    ...
    if (!isempty(gen_ind)) {
      ...
      while (iter <= iterations) {
        ...
        base = calculate.crit(edge_ind_base, flim_base, f_base, 1.5)
        if (base == 1) {
          flows[, iter] = f_base
          iter = iter + 1
          crit_base = crit_base + 1
          results[,crit_base] = out.of.bounds(edge_ind, flim, f, 1.5)
        }
      }
    }
  }
...
#---------------------------------------------------------------------------------------------------------------------------------------------------
write.csv(flows, "flows_1.5_3007.csv", row.names = F)
write.csv(results, "results_1.5_3007.csv", row.names = F)
