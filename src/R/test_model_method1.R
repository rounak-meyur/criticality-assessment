...
handle.islands = function(bus, branch, gen, dat) {
  ...
  for(t in 1:length(gs)) {
    ...
    # max.demand = sum(dat$dmax[bus_ind])
    # C = gen.bus.con(bus, gen)
    # min.generation = sum(as.matrix(C %*% dat$gmin)[bus_ind])
    # if((length(bus_ind) > 1) & (max.demand < min.generation)) {
    #   bus$type[busind] = 4
    #   dat$dmax[busind] = 0
...
for (line_removal in 2571:nrow(branchdat)) {
...
print(proc.time() - ptm)
write(criticality, "criticality_1.5_1")
