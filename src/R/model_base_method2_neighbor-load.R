...
full_model = function(rate, rateA_all, neighbor, load, file) {
  if (neighbor) {
    check.criticality = read.delim2(file, header = FALSE, stringsAsFactors = FALSE)
  }
...
#---------------------------------------------------------------------------------------------------------------------------------------------------
criticality1.5N = full_model(seq(1.1, 3, 0.1), TRUE, FALSE, "~/check-criticality-1.5.txt")
criticality1.25N = full_model(seq(1.1, 3, 0.1), 1.25, TRUE, FALSE, "~/check-criticality-1.25.txt")
